...
{ const d=fs.readFileSync(path.join(root,'src/utils/diagnostics.ts'),'utf8'); const m=d.match(/KIZILKAN_FLIGHT_RECORDER_V(\d+)/); const persistent=/KIZILKAN_(?:DIAGNOSTICS_V1|BLACK_BOX_V2)/.test(d)||(m&&Number(m[1])>=3); if(!persistent){ console.log('HATA — kalıcı tanılama raporu formatı yok'); bad++; } }
