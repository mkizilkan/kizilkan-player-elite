...
(async()=>{
 ...
 const calls=[];const iptv={xtreamLogin:async()=>({user_info:{status:'Active'}}),xtreamLiveStreams:async()=>{calls.push('live');return[{id:'l'}];},xtreamVod:async()=>{calls.push('vod');return[{id:'v'}];},xtreamSeries:async()=>{calls.push('series');return[{id:'s'}];},fetchAndParseM3U:async()=>({channels:[{id:'l'}],vod:[{id:'v'}],series:[{id:'s'}]})};
 ...
 r=await refresh({...pl,source:'m3u_url',m3uUrl:'http://example.com/list'},undefined,{kinds:['series']});assert.equal('channels' in r.patch,false);assert.equal(r.patch.series.length,1);
