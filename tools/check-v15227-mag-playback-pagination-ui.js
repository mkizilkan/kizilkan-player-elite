...
function staticChecks(){
  ...
  need(player,/STALKER_PLAYBACK_SOURCE_RENEW/,'HTTP stale-source renew telemetry yok');
  need(player,/shouldRenewResolvedSource/,'HTTP stale-source renew policy PlayerHost içine bağlı değil');
  need(recovery,/status === 401[\s\S]*status === 403[\s\S]*status === 444[\s\S]*status === 456[\s\S]*status === 520/,'401/403/444/456/520 renew sınıflandırması yok');
  need(recovery,/status === 404[\s\S]*return "not_found"/,'404 not_found ayrımı yok');
...
function fixtureSourceRecoveryContract(){
  const js=transpile(recovery,'sourceRecovery.ts');
  const crypto={CryptoDigestAlgorithm:{SHA256:'SHA256'},digestStringAsync:async()=> 'f'.repeat(64)};
  const box={module:{exports:{}},exports:{},require:id=>id==='expo-crypto'?crypto:require(id),console}; box.exports=box.module.exports;
  vm.runInNewContext(js,box,{filename:'sourceRecovery-v15227.js'});
  const api=box.module.exports;
  for(const status of [401,403,444,456,520]) if(!api.shouldRenewResolvedSource(status,'stalker_create_link')) throw new Error('stale-source renew eksik HTTP '+status);
  if(api.shouldRenewResolvedSource(404,'stalker_create_link')) throw new Error('404 yanlışlıkla session renew oluyor');
  if(api.classifyHttpRecovery(404)!=='not_found') throw new Error('404 not_found sınıfı bozuk');
  if(api.shouldRenewResolvedSource(401,'direct')) throw new Error('direct URL yanlışlıkla Stalker renew oluyor');
}
...
(async()=>{try{staticChecks();fixtureSourceRecoveryContract();transpile(player,'PlayerHost.tsx',true);transpile(add,'add-playlist.tsx',true);transpile(request,'request.ts');fixturePlaybackRequestBridge();await fixtureOneBasedAlias();await fixtureZeroBasedPagination();await fixtureIgnoredPageGovernor();await fixtureRecoveryCreateLink();await fixturePlaybackHeaders();await fixtureExternalHostRedaction();console.log('TEMIZ — v15.2.27 MAG playback context + stale-source renew + adaptive pagination + add progress + emergency controls fixtures');}catch(e){console.error('HATA — v15.2.27 P0 gate:',e&&e.stack||e);process.exit(1)}})();
