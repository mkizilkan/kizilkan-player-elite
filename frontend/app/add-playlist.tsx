...
export default function AddPlaylist() {
  ...
  const runNativeStreamingBulkFile = async (
    ...
    try {
      ...
      if (signal?.aborted || bulkScanCancelledRef.current) return { found:0, completed:0, cancelled:true };
      // v17.4.0: kapsam filtresi her yolda uygulanır
      const baseDirectory = await resolveScanDirectory(src, { signal, timeoutMs: cfg.timeoutMs });
      if (signal?.aborted || bulkScanCancelledRef.current) return { found:0, completed:0, cancelled:true };
      /**
       * v17.9.4 — DOĞRUDAN DNS'LER STREAMING YOLUNA DA EKLENİYOR
       * -------------------------------------------------------------------
       * KÖK NEDEN (kullanıcı 22.09): İki ayrı combo yolu var. Küçük listelerde
       * runNativeBulkAccounts (v17.9.2'de doğrudan DNS bağlandı), BÜYÜK
       * dosyalarda ise startStreamingFileScanV172 çalışır. Kullanıcı büyük
       * dosya seçince streaming yol devreye girdi ve bu yol YALNIZ rehberi
       * (baseDirectory) taradı; girilen DNS'ler eklenmedi. Log kanıtı:
       * hesap başına ~11 adres denendi (2 DNS beklenirken), BULK_SCAN_PLAN
       * hiç yazılmadı (o telemetri sadece 1. yolda).
       *
       * Artık doğrudan DNS'ler, rehberde OLMASALAR BİLE tek bir sanal panel
       * olarak directory'ye eklenir; native tarayıcı her hesabı bu DNS'lerde de
       * dener. Tarama hızı/paralellik ayarları (requestedConcurrency, batchSize,
       * timeoutMs) bu panele de AYNEN uygulanır — ayrı bir yol yok.
       */
      const directHostsForStream = validPanelHosts(panelTarget.hosts || []);
      const directory = directHostsForStream.length
        ? [...baseDirectory, { panelName: "Doğrudan DNS", code: "", hosts: directHostsForStream, sources: [] } as any]
        : baseDirectory;
      void recordDiagnostic("scan", "V173_STREAM_DIRECTORY_READY", {
        panels: directory.length, hosts: directory.reduce((n,p)=>n+p.hosts.length,0),
        directHosts: directHostsForStream.length, fileBytes: source.size,
      }, { stage:"scan-prepare" });
      const requestedConcurrency = Math.max(1, Math.min(250, cfg.requestedConcurrency ?? (cfg.concurrency * Math.max(1, cfg.accountConcurrency))));
