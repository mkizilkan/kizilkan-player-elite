...
import { refreshPlaylistContent, formatRefreshProgress } from "@/src/utils/refreshPlaylist";
...
export default function PlaylistSelect() {
  ...
  useEffect(() => {
    ...
    if (sorted.length === 1) {
      ...
      navigationStartedRef.current = true;
      (async () => {
        // v17.9.10: activeId aynı olsa bile Room snapshot boş/kayıp olabilir.
        // Her giriş gerçek setActivePlaylist doğrulamasından geçer.
        await setActivePlaylist(only.id);
    ...
    autoTimeoutRef.current = setTimeout(() => {
      ...
      (async () => {
        const last = sorted[0];
        // Son kullanılan meta aktif görünse bile canonical Room doğrulanır.
        await setActivePlaylist(last.id);
        router.replace(homeRoute as any);
      })().catch((e:any) => {
        navigationStartedRef.current = false;
        Alert.alert('Playlist seçilemedi', String(e?.message || e || 'Liste içeriği doğrulanamadı.'));
  ...
  const enterList = async (id: string) => {
    if (navigationStartedRef.current) {
      // v17.9.10: sessiz kara delik yok. Recovery varsa global overlay zaten
      // gerçek aşamayı gösterir; normal geçişte de kullanıcıya kısa bilgi ver.
      if (!heavyLoading) Alert.alert("Liste açılıyor", "Önceki seçim işlemi tamamlanıyor. Lütfen kısa bir süre bekleyin.");
      return;
    }
    navigationStartedRef.current = true;
    try {
      // v17.9.10: aynı activeId, içeriğin Room'da gerçekten var olduğu anlamına
      // gelmez. Her kullanıcı girişi verified activation zincirinden geçer.
      ...
      /**
       * v17.9.8 — OOM (bellek yetersizliği) AYRI ELE ALINIR
       * SORUN (23.09 logu): Büyük katalog (13K+ kanal) onarımı 75 MB tek blok
       * ayırmaya çalışıp OutOfMemoryError ile çöküyordu. Bu "boş kabuk" deseni
       * değil; kullanıcıya farklı ve doğru bir mesaj gösterilir. Ayrıca hangi
       * hata olursa olsun seçim kilidi (refreshOneProgress/heavyLoading)
       * PlaylistContext finally'lerinde temizlendiği için burada takılı kalmaz;
       * yine de güvence olsun diye navigationStartedRef sıfırlandı (yukarıda).
       */
      const isOOM = /OutOfMemory|allocate|footprint|bellek/i.test(msg);
      if (isOOM) {
        /**
         * v17.9.9 — DOĞRU OOM MESAJI. v17.9.8'deki "diğer uygulamaları kapatın"
         * YANLIŞTI: sorun cihaz RAM'i (16 GB) değil, uygulamanın Android Java
         * heap limiti (~384 MB). Başka uygulama kapatmak etkilemez. v17.9.9
         * parçalı gönderim + largeHeap ile bunu büyük ölçüde çözer; yine de
         * aşırı büyük listede kategorileri sınırlamak kesin çözümdür.
         */
        Alert.alert(
          "Liste çok büyük",
          "Bu liste çok fazla içerik barındırıyor. Yeni sürüm belleği çok daha " +
          "verimli kullanıyor; yine de sorun sürerse listeyi düzenleyip yalnızca " +
          "ihtiyacınız olan kategorileri (kanal/film/dizi) seçerek boyutu " +
          "küçültebilirsiniz.",
        );
        return;
      }
  ...
  const choose = async (id: string) => {
     ...
     */
    /**
     * v17.9.8 — Kilit yalnız GÖRÜNÜR yenileme (refreshingId) sürerken geçerli.
     * Eskiden heavyLoading'e de bakıyordu; ama setActivePlaylist içindeki onarım
     * OOM ile çökerse heavyLoading kısa süre açık kalıp kilidi yanlışlıkla
     * tetikleyebilirdi. refreshingId, ekranda dönen göstergesi olan gerçek
     * kullanıcı-başlatılı yenilemeyi işaret eder ve finally'de kesin temizlenir.
     */
    if (refreshingId) {
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: colors.surface }]} edges={["top"]} testID="playlist-select-screen">
      ...
      {/**
        ...
        */}
      {/* v17.9.10: Otomatik boş-kabuk recovery artık _layout içindeki global
          PlaylistRepairOverlay tarafından gösterilir. Burada yalnız kullanıcı
          tarafından başlatılan manuel yenilemenin mevcut katmanı kalır. */}
      {!!refreshOneProgress && (
        <View style={{
          position: "absolute", top: 0, left: 0, right: 0, bottom: 0,
          backgroundColor: "rgba(0,0,0,0.90)", alignItems: "center", justifyContent: "center",
          ...
          <Text style={{color:colors.onSurfaceSecondary,fontSize:FONT.size.lg,textAlign:"center",marginTop:SPACING.md,lineHeight:26}}>{refreshOneProgress}</Text>
          <Text style={{color:colors.brandPrimary,fontSize:FONT.size.base,fontWeight:"700",textAlign:"center",marginTop:SPACING.xl}}>
            Güncelleme devam ediyor. Tamamlanana kadar bu ekranda bekleyin.
          </Text>
        </View>
      )}
    </SafeAreaView>
