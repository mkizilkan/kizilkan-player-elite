...
export interface Playlist {
  ...
  catalogCapabilities?: {
    ...
    vod?: 'supported' | 'empty' | 'unsupported_404' | 'error';
    series?: 'supported' | 'empty' | 'unsupported_404' | 'vod_fallback' | 'error';
    updatedAt?: string;
  };
  /** v16.14.2: client snapshot diff V2. Server-delta DEĞİLDİR. Fingerprint yalnız doğrulanmış Room commit sonrası publish edilir. */
  catalogSync?: {
    liveFingerprint?: string;
    vodFingerprint?: string;
    seriesFingerprint?: string;
    lastChangedKinds?: Array<'live' | 'vod' | 'series'>;
  ...
  magCapabilities?: {
    live?: 'supported' | 'empty' | 'error';
    vod?: 'supported' | 'empty' | 'unsupported' | 'error';
    series?: 'supported' | 'empty' | 'vod_fallback' | 'unsupported' | 'error';
    profile?: 'supported' | 'empty' | 'error';
    model?: string;
    transport?: 'native_okhttp' | 'fetch' | 'unknown';
