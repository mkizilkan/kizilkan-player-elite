...
export async function refreshPlaylistContent(pl: Playlist, onProgress?: (p: RefreshProgress) => void, options?: RefreshOptions): Promise<RefreshResult> {
  ...
  try {
    if (pl.source === "xtream") {
      ...
      if (options?.incrementalDelivery) {
        ...
        const fetchers:Record<CatalogKind,()=>Promise<any[]>>={
          live:()=>xtreamLiveStreams(cred),
          vod:()=>xtreamVod(cred),
          series:()=>xtreamSeries(cred),
        };
      ...
      const livePromise=requested.has('live')?timed('live',()=>xtreamLiveStreams(cred)):Promise.resolve([]);
      const vodPromise=requested.has('vod')?timed('vod',()=>xtreamVod(cred)):Promise.resolve([]);
      const seriesPromise=requested.has('series')?timed('series',()=>xtreamSeries(cred)):Promise.resolve([]);
    ...
    if (pl.source === "stalker") {
      ...
      const selected = applyContentSelection({ channels: catalog.channels, vod: catalog.vod, series: catalog.series }, options?.ignoreContentSelection ? null : pl.contentSelection);
