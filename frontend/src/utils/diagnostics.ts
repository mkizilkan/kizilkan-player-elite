...
export type FlightRecorderV7Stage = 'playlistSelect' | 'roomVerify' | 'catalogRecovery' | 'channelSelect' | 'urlResolve' | 'enginePrepare' | 'httpResponse' | 'fallback' | 'firstFrame';
export function createFlightRecorderTrace(prefix: string, seed = ''): string {
  const entropy = `${Date.now()}-${Math.random()}-${seed}`;
  return `${prefix}-${shortHash(entropy)}-${Date.now().toString(36)}`;
}
/**
 * v16.14.3 — Parent/child correlation. Playlist trace global parent olarak kalır;
 * her kanal seçimi kendi child trace'ini alır. Böylece aynı playlist altındaki
 * Kanal A/B/C engine/url/fallback/firstFrame olayları tek trace içinde karışmaz.
 */
export function createFlightRecorderChildTrace(parentTraceId: string, prefix: string, seed = ''): string {
  const child = createFlightRecorderTrace(prefix, `${parentTraceId}|${seed}`);
  return parentTraceId ? `${parentTraceId}>${child}` : child;
}
export function beginFlightRecorderTrace(prefix: string, seed = ''): string {
  currentFlightV7TraceId = createFlightRecorderTrace(prefix, seed);
  return currentFlightV7TraceId;
}
export function getCurrentFlightRecorderTrace(): string { return currentFlightV7TraceId; }
export function clearFlightRecorderTrace(traceId?: string): void {
  if (!traceId || currentFlightV7TraceId === traceId) currentFlightV7TraceId = '';
}
export async function recordFlightRecorderStage(traceId: string, stage: FlightRecorderV7Stage, data: Record<string, any> = {}, outcome = ''): Promise<void> {
  return recordDiagnostic('lifecycle', 'FLIGHT_V7_STAGE', { lifecycleStage: stage, ...data }, { traceId, stage, ...(outcome ? { outcome } : {}) });
}
