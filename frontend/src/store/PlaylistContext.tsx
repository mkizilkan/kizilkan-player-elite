  ...
  const refreshPlaylistKinds=useCallback(async(id:string,kinds:Array<CatalogKind|'epg'>,progress?:(p:RefreshProgress)=>void,valid:()=>boolean=()=>true,forceUnconditional=false)=>{
    ...
    return withCatalogLock(id,async()=>{
      ...
      if(catalogKinds.length){
        ...
        try{
          const {buildRefreshDiff,formatRefreshDiff,diffTelemetry}=await import('@/src/utils/refreshDiff');
          const after={
          ...
          const diff=buildRefreshDiff(beforeSnapshot,after);
          void recordDiagnostic('catalog','PLAYLIST_REFRESH_DIFF',{playlistId:id,...diffTelemetry(diff,forceUnconditional?'manual':'auto')});
  ...
  const setActivePlaylist = useCallback(async (id: string) => {
    ...
    if (KizilkanNativeCore.available) {
      ...
      try {
        ...
        if (verifiedTotal === 0) {
          const metaTotal = (target.channelsCount || 0) + (target.vodCount || 0) + (target.seriesCount || 0);
          void recordDiagnostic('catalog', 'PLAYLIST_EMPTY_SHELL_DETECTED', {
            playlistId: id,
            source: target.source || '',
            metaTotal,
            hasSource: !!(target.m3uUrl || target.xtreamServer || target.stalkerPortal),
          });
