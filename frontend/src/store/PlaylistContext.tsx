...
export type PlaylistRepairProgress = {
  ...
  phase: "verify" | "dns" | "login" | "content" | "save" | "roomVerify" | "ready" | "error";
  message: string;
...
export function PlaylistProvider({ children }: { children: React.ReactNode }) {
  ...
  const repairMissingPlaylist = useCallback(async (
    ...
    const publishState=(phase:PlaylistRepairProgress['phase'],message:string,progress:RefreshProgress|null=lastProgress,partial?:boolean)=>{
      const now=Date.now();
      setRepairProgress({playlistId:id,playlistName,source:broken.source||'',mode,phase,message,progress,startedAt,lastProgressAt:now,partial});
    };
    const onProgress=(p:RefreshProgress)=>{
      ...
      setRepairProgress({playlistId:id,playlistName,source:broken.source||'',mode,phase,message:p.message,progress:p,startedAt,lastProgressAt:now});
    ...
    try{
      ...
      const incrementalDelivery = broken.source==='xtream' ? async (delivery:IncrementalCatalogDelivery) => {
        if(activeSwitchGeneration.current!==generation)throw new Error('Playlist seçimi değişti; eski onarım sonucu uygulanmadı.');
        const commitStarted=Date.now();
        const patch:Partial<Playlist>=delivery.kind==='live'
          ? {channels:delivery.items as Playlist['channels']}
          : delivery.kind==='vod'
            ? {vod:delivery.items as Playlist['vod']}
            : {series:delivery.items as Playlist['series']};
        await updatePlaylist(id,{
          ...patch,
          catalogRecovery:{state:'running',startedAt,expectedKinds,completedKinds:Array.from(completedKinds)},
          lastRefreshOk:false,
        });
        ...
        void recordDiagnostic('catalog','PLAYLIST_REPAIR_KIND_COMMITTED',{
          playlistId:id,kind:delivery.kind,rawCount:delivery.rawCount,filteredCount:delivery.filteredCount,
      ...
      const res=await refreshPlaylistContent(broken,onProgress,incrementalDelivery?{
        incrementalDelivery,
        allowPartialRecovery:true,
        // Boş kabukta yerel içerik YOK; M3U validator 304 dönerse kurtarma
        // yapılamaz. Recovery her kaynakta gerçek içeriği yeniden istemelidir.
      ...
      void recordDiagnostic('catalog',res.partial?'PLAYLIST_SELF_REPAIR_PARTIAL':'PLAYLIST_SELF_REPAIR_OK',{
        playlistId:id,mode,elapsedMs:Date.now()-startedAt,channels:summary.channels||0,vod:summary.vod||0,series:summary.series||0,
      ...
      void recordDiagnostic('catalog','PLAYLIST_SELF_REPAIR_ERROR',{playlistId:id,mode,error:message,elapsedMs:Date.now()-startedAt},{traceId,stage:'catalogRecovery',outcome:'failed'});
