...
export function PanelScopePicker({scope,target,directory,onScope,onTarget,onLoad,busy}:{scope:DirectoryScope;target:PanelTarget;directory:DirectoryPanel[];onScope:(s:DirectoryScope)=>void;onTarget:(t:PanelTarget)=>void;onLoad:()=>void;busy:boolean}){
 ...
 /**
  * v17.4.1 — 100 PANEL SINIRI BU BİLEŞENDE DE KALDIRILDI.
  * v17.4.0'da add-playlist.tsx içindeki liste düzeltilmişti, fakat kullanıcının
  * gördüğü "Panel listesi" bu bileşendir; burada sınır durduğu için ekranda hâlâ
  * "794 eşleşme · ilk 100 gösterilir" yazıyordu. Aynı hatanın iki ayrı yerde
  * olması, bu iki listenin ortak bir bileşene taşınması gerektiğini gösteriyor
  * (ayrı madde olarak not edildi).
  *
  * Arama artık DNS ADRESLERİNDE de çalışır ve çok kelimelidir; kullanıcı panelin
  * adını bilmese de adresinden bulabilir.
  */
 const matches=directory.filter(p=>{
   const q=search.trim().toLocaleLowerCase('tr');
   if(!q)return true;
   const terms=q.split(/\s+/).filter(Boolean);
   const hay=[p.panelName,...(p.codes||[]),...(p.hosts||[]),...(p.sources||[]).map(s=>s.source)].join(' ').toLocaleLowerCase('tr');
   return terms.every(t=>hay.includes(t));
 });
 const [visibleCount,setVisibleCount]=useState(100);
 React.useEffect(()=>{setVisibleCount(100);},[search,scope]);
 ...
 {expanded&&<><View style={{flexDirection:'row',flexWrap:'wrap',gap:8}}>{button(directory.length?`Rehberi yenile (${directory.length})`:'Rehberi yükle',false,onLoad)}</View><TextInput value={search} onChangeText={setSearch} placeholder="Panel adı, kod veya kaynak ara" placeholderTextColor={colors.onSurfaceTertiary} style={inputStyle}/><ScrollView nestedScrollEnabled style={{maxHeight:270}} keyboardShouldPersistTaps="always">{shown.map(p=>{const key=p.directoryKey||p.panelName,checked=target.keys?.includes(key)||false;const codes=(p.codes||[]).filter(Boolean);const sources=Array.from(new Set((p.sources||[]).map(s=>s.source)));return button(`${checked?'☑':'☐'} ${p.panelName}${codes.length?` · Kod: ${codes.join(', ')}`:' · Kod yok'} · ${sources.join(' + ')||'Kaynak bilinmiyor'} · ${p.hosts.length} DNS`,checked,()=>onTarget({...target,keys:checked?target.keys?.filter(k=>k!==key):[...(target.keys||[]),key]}));})}</ScrollView><Text style={{color:colors.onSurfaceSecondary,fontSize:12}}>{`Toplam ${directory.length} panel · eşleşen ${matches.length} · gösterilen ${shown.length} · seçili ${target.keys?.length||0}`}</Text>
 {matches.length>shown.length&&button(`Daha fazla göster (${matches.length-shown.length} panel daha)`,false,()=>setVisibleCount(c=>c+200))}
 {matches.length===0&&<Text style={{color:colors.onSurfaceSecondary,fontSize:12}}>Aramanıza uyan panel yok. Ad, kod, kaynak veya DNS deneyin.</Text>}</>}
