...
class PanelScanService : Service() {
  ...
  override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
    when (intent?.action) {
      ACTION_CANCEL -> {
        cancelled.set(true)
        writeSnapshot(JSONObject().put("running", false).put("cancelled", true))
      ...
      ACTION_START -> if (!running) {
        ...
        val concurrency = intent.getIntExtra("concurrency", 6).coerceIn(1,12)
        ...
        writeSnapshot(JSONObject()
          .put("running", true).put("tested", 0).put("total", initialTotal)
  ...
  private fun runScan(raw: String, user: String, pass: String, concurrency: Int, timeoutMs: Int) {
    try {
      ...
      repeat(concurrency) {
        pool.submit {
          while (!cancelled.get()) {
            ...
            val snap = JSONObject()
              .put("running", done < total && !cancelled.get())
      ...
      synchronized(matches) { matches.forEach { resultArray.put(it) } }
      writeSnapshot(JSONObject()
        .put("running", false).put("tested", tested.get()).put("total", total)
        .put("panelTested", panelDone.get()).put("panelTotal", panelTotal)
