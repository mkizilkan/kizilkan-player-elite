...
export const KizilkanNativeCore = {
  ...
  queryItems: async <T = any>(id: string, kind: "live" | "vod" | "series", opts?: { group?: string; search?: string; offset?: number; limit?: number }): Promise<NativeQueryPage<T>> => {
    if (!native) return { items: [], offset: 0, returned: 0, hasMore: false };
    return native.queryItems(id, kind, opts?.group || "__all__", opts?.search || "", opts?.offset || 0, opts?.limit || 100);
  },
  ...
  clearCache: () => native ? native.clearCache() : false,
